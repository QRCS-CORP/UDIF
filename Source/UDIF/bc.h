...
/*!
 * \file bc.h
 * \brief UDIF Branch Controller server public API.
 *
 * Dual-role: QSTP client to Root + QSTP server to GCs.
 * All protocol messages are handled by the shared dispatch/handler layer.
 * The operator console runs on the main thread.
 *
 * Entry point from appbc.c:
 *   int main(void) { udif_bc_start_server(); return 0; }
 */
