...
/*!
 * \file gc.h
 * \brief UDIF Group Controller server public API.
 *
 * Dual-role: QSTP client to BC + QSTP server to UAs.
 * All protocol messages are handled by the shared dispatch/handler layer.
 * The operator console runs on the main thread.
 *
 * Entry point from appgc.c:
 *   int main(void) { udif_gc_start_server(); return 0; }
 */
